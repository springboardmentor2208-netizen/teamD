  ...
  return (
    <nav ref={navRef} className="sticky top-0 z-[100] bg-slate-950 text-white border-b border-slate-800 shadow-2xl">
      <div className="w-full max-w-[1600px] mx-auto px-6 md:px-10 py-5 flex justify-between items-center">
        ...
        {/* LEFT: LOGO & NAME - Cleaned up to show logo as a pure icon */}
          ...
          <div className="hidden lg:flex items-center gap-1 bg-slate-900/50 p-1 rounded-2xl border border-slate-800 ml-auto mr-10">
            {[
              ...
              <Link
                key={link.path}
                to={link.path}
                className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${isActive(link.path) ? "bg-blue-600 text-white shadow-lg" : "text-slate-400 hover:text-white hover:bg-slate-800"
                  }`}
              >
                {link.name}
              </Link>
            ...
            {/* VOLUNTEER DESKTOP LINK */}
            {user.role === "volunteer" && (
              <Link
                to="/my-tasks"
                className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${isActive("/my-tasks") ? "bg-amber-500 text-black shadow-lg" : "text-amber-500 hover:bg-amber-500/10"
                  }`}
              >
                My Tasks
              </Link>
        ...
        <div className="flex items-center gap-6 flex-shrink-0 ">
          {!user ? (
            ...
            <div className="flex items-center gap-4 relative">
              ...
              >
                <img
                  src={user.profilePhoto || `https://ui-avatars.com/api/?name=${user.fullName}&background=2563eb&color=fff`}
                  alt="Avatar"
                  className="w-10 h-10 rounded-2xl object-cover border-2 border-slate-800 shadow-lg"
                />
                <div className="hidden md:block text-left">
                  <p className="text-[10px] font-black leading-none mb-1 uppercase tracking-tighter">{user.fullName.split(' ')[0]}</p>
              ...
              {profileOpen && (
                <div className="absolute right-0 top-16 w-60 bg-white rounded-[2rem] shadow-2xl border border-slate-100 overflow-hidden animate-in fade-in slide-in-from-top-4 duration-200">
                  ...
                  <div className="p-2">
                    ...
                    {/* ADDED: Volunteer Task shortcut in dropdown */}
                    {user.role === "volunteer" && (
